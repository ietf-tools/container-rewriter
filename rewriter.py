    ...
    def eom(self):
        try:
            ...
            # no scenario match
                ...
                rewrite_domain = rewrite_domain_map.get(env_from_addr.split("@")[1])
                if rewrite_domain is None:
                    logging.info(f"{queue_id} none: sender {env_from_addr} not a rewrite domain, no action [{self.id}]")
                    return Milter.CONTINUE
